...
1. We're going to be using the MacOS **Terminal** application as our Unix command shell. We'll be using it often so it would be wise to copy the icon for the Terminal application 
   ...
   the part `rm:~\>` is the prompt.

   > **Heads up! In the bash commands below, we'll omit the prompts to simplify cutting and pasting.**
...
2. Most operating systems use a tree-structured hierarchy of *directories* (aka *folders*) for organizing resources. Each user is assigned a *home directory* where they can organize their files and subdirectories. When a user is interacting with the operating system via a command shell, In addition to a home directory, the user will also have a *current working directory*.
   ...
   The *make directory* command, `mkdir` introduces a new directory in the current working directory. Let's make a work area for the materials developed in this class. (We'll sometimes refer to this new directory as the *course home directory*.)
...
5. Initialize `opam`, the OCaml Package Manager

   ```bash
   opam init
   ```

   > Heads up! Best to copy and paste the following command -- the two quote marks in the `eval` command below are *backquotes* (upper left, below the `esc` key). This won't work if you type the normal single-quote marks.
...
6. The process of installing OCaml's package manager `opam`, also installs the other main parts of the OCaml system. Fire up the OCaml interpreter to confirm that it's working.
   ...
   > It's fine if you have OCaml 4.11 instead.
...
1. First, install several of Ubuntu's packages. 
   ...
   > Heads up! 
   >
   > 1. Probably best to just copy the first line and paste it into your command shell. 
   > 2. The first use of sudo will require you to type your new Ubuntu password.
...
5. Now execute the course `configure` script to install several OCaml packages: `dune`, `merlin`, `animate`, etc. Type Y to accept the installation.
...
```bash
cd /mnt/c/YOUR_WINDOWS_USERNAME/
mv Documents/configure.xlaunch AppData\Roaming\Microsoft\Windows\Start Menu\Programs\Startup\
```

>  Heads up! `YOUR_WINDOWS_USERNAME` is your Windows user name, not your Ubuntu name! If you don't know what your Windows user name is, go to the *Type here to search* area in Windows (lower left) and type `cmd` Enter. If it shows `C:\Users\abraham>` then `abraham` is your Windows user name.

If you save your `config.xlaunch` file in the `Startup` directory as shown, then the X-server will start automatically for you on system startup and you won't have to hassle with starting it by hand anymore. Once the X-server is launched, an X icon will appear in your system tray. If you want to run a graphical application and you don't see the X icon in the starter tray, you can start the X-server by hand by double-clicking the XLaunch icon.
